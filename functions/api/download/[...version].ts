...
async function getR1Usage(env: Env): Promise<number> {
  let used = 0
  const listed = await env.R1.list()
...
async function tryCacheToR1(env: Env, key: string, body: ArrayBuffer, contentType: string) {
  const maxCapacity = parseInt(env.R1_MAX_CAPACITY || '10737418240', 10)
  const currentUsage = await getR1Usage(env)

  if (currentUsage + body.byteLength > maxCapacity) {
    const victimKey = await findLeastUsedFile(env)
    if (victimKey && victimKey !== key) {
      await env.R1.delete(victimKey)
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const r1Obj = await env.R1.get(key)
  if (r1Obj) {
  ...
  waitUntil((async () => {
    ...
    await tryCacheToR1(env, key, combined.buffer, contentType)
    const countKey = `count:${key}`
